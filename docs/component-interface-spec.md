...
> **Authority:** this document defines the exact interface contracts for the lab environment's three public interfaces: the HTTP service API, the application log schema, and the state machine transition map. It is the formal specification that the conformance suite enforces and the CLI parses.
>
...
The subject service is a Go HTTP server bound to `127.0.0.1:8080`. All conformance-relevant traffic is proxied through nginx. Three endpoints are defined. No authentication. No request body is accepted by any endpoint.
...
No other fields are ever present. The body is a string literal — it is not produced by `json.Marshal`. It never varies.
...
**Fault targets:** F-004 (state dir mode 000 → 500), F-018 (inode exhaustion → 500).
...
**The `env` field is absent in the error response.** The two response shapes are distinguished by `status` value and by the presence/absence of `env`/`msg`. No other 5xx response is produced by this handler.
...
**Purpose:** baseline network behaviour demonstration (B-001). Not part of the conformance suite.
...
These are all log entries the service is defined to emit. The `msg` value is the exact string used.
...
| `"listening"` | info | `addr` (string) | — | Step 9, after socket bound |
...
|---|---|---|---|
| `"chaos modes active"` | warn | `modes` (string, comma-joined mode names) | Startup, when any chaos mode is active |
| `"SIGTERM ignored (CHAOS_IGNORE_SIGTERM=1) — process will require SIGKILL"` | warn | none | Startup, when F-008 fault flag is active |
| `"chaos drop"` | warn | `path` (string), `drop_percent` (int) | Each dropped request (if logger non-nil) |

#### Request error entries

| msg | level | Additional fields | Conformance relevance | When emitted |
|---|---|---|---|---|
| `"state write failed"` | error | `path` (string: `/var/lib/app/state`), `error` (string) | E-002 fails when this is logged | F-004 or F-018 active, on each `GET /` |
...
#### Startup error entries (written before crash)
...
| `CONFORMANT` | All 23 blocking checks pass. `active_fault` is null. | All commands |
...
| F-008 | SIGTERM ignored | _(none while running)_ | All 23 pass | R3 |
...
| F-014 | Zombie accumulation | _(none initially)_ | All 23 pass | R3 |
...
**F-008 and F-014 notes:** `lab validate` exits 0 while these faults are active. They are in DEGRADED state (fault ID recorded) but the conformance suite cannot observe them during normal operation. F-008 manifests at shutdown (SIGTERM ignored); F-014 manifests over time (zombie accumulation).
...
|---|---|---|---|
| F-001–F-007, F-009, F-013, F-015–F-018 | no | none | N/A |
...
| E-002 only; E-001 passes | F-004, F-018 | `df -i /var/lib/app` — inodes → F-018; mode → F-004 |
| E-series only; P-002 passes | F-007 | `cat /etc/nginx/sites-enabled/app \| grep server` |
...
| None; `lab validate` exits 0 | F-008, F-014 | `time systemctl stop app` / `ps aux \| grep Z` |
...
**`inode_usage_percent` fault signal:** this field approaches 100.0 during F-018 (inode exhaustion) while `disk_usage_percent` remains low. This is the telemetry-based complement to `df -i /var/lib/app`.
...
| `Unhealthy` | State directory write fails; `/` returns 500 | On F-004/F-018 write failure |
