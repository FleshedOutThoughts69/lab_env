#!/usr/bin/env bash
# /opt/lab-env/scripts/bootstrap.sh
...
step "02b-go-version"
GO_EXE=$(command -v go) || fail "go binary not found after package install"
GO_VER=$("${GO_EXE}" version | awk '{print $3}' | sed 's/go//')
...
if [[ "${MAJOR}" -lt "${REQUIRED_GO_MAJOR}" ]] ||
   [[ "${MAJOR}" -eq "${REQUIRED_GO_MAJOR}" && "${MINOR}" -lt "${REQUIRED_GO_MINOR}" ]]; then
    fail "Go ${GO_VER} is too old; need ${REQUIRED_GO_MAJOR}.${REQUIRED_GO_MINOR}+. Install manually from https://go.dev/dl/"
fi
log "Go ${GO_VER} OK"
...
visudo -c -f "${TMP_SUDOERS}" || { fail "sudoers syntax error"; }
...
# ── Run validate.sh to confirm CONFORMANT ─────────────────────────────────────
VALIDATE_SCRIPT="${LAB_DIR}/scripts/validate.sh"
if [[ -x "${VALIDATE_SCRIPT}" ]]; then
    ...
    log "WARNING: validate.sh not found at ${VALIDATE_SCRIPT}; run 'lab validate' manually"
fi